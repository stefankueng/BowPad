        ...
        (wParam == '-'))
    {
        ...
        else
        {
            ...
            else if ((wParam == '[') || (wParam == '-'))
            {
                ...
                FindResult result1    = FindText("/", currentPos, 0, 0);
                FindResult result2    = FindText("<", currentPos, 0, 0);
                if (result2.success)
                {
    ...
    else if (wParam == '>')
    {
        ...
        if (result2.success)
        {
            if (!result1.success || (result2.start > result1.start))
            {
                ...
                Call(SCI_ADDTEXT, 1, reinterpret_cast<sptr_t>(">"));
                auto cursorPos = Call(SCI_GETCURRENTPOS);
                // find the tag id
                std::string tagName;
                auto        position = result2.start + 1;
                ...
                if (!tagName.empty())
                {
                    ...
                    else
                    {
                        ...
                        Call(SCI_ADDTEXT, 2, reinterpret_cast<sptr_t>("</"));
                        Call(SCI_ADDTEXT, tagName.size(), reinterpret_cast<sptr_t>(tagName.c_str()));
                        Call(SCI_ADDTEXT, 1, reinterpret_cast<sptr_t>(">"));
                    }
                    Call(SCI_GOTOPOS, cursorPos);
                    Call(SCI_ENDUNDOACTION);
